...
import io.bullet.borer.{Cbor, Dom, Encoder, Writer}
 ...
 *   `(CborOptions, ConstructorEncoder, Encoder[F1], Encoder[F2], …)` where `F1, F2, …` are the unique
 *   field types across all constructors of `T`. At runtime, the entry's closure matches on the runtime
 *   value and packs a [[FromConstructor]] that the [[ConstructorEncoder]] turns into `Dom.Element`. The
 *   resulting borer `Encoder[T]` materializes that DOM into CBOR bytes on call.
 *
 *   For sum types (sealed traits / enums), one pattern-match branch is generated per child constructor.
 *
 *   The macro emits the fully-qualified constructor name. The presentation of that name in the CBOR
 *   tag is controlled by [[CborOptions.constructorTagModifier]] (default: drop the qualifier).
 *
 ...
 *   dispatches to the real encoder once it exists.
 */
...
/** Convert any value to a `Dom.Element` using its borer `Encoder`. Used internally by the macros. */
private[cbor] def toElement[A](e: Encoder[A], a: A): Dom.Element =
  Cbor.decode(Cbor.encode(a)(using e).toByteArray).to[Dom.Element].value

private[cbor] object EncoderMacro:
  ...
  def valueImpl[X: Type](x: Expr[X])(using q: Quotes): Expr[Registry[? <: Tuple, ? <: Tuple]] =
    ...
    def asEncoderType(t: TypeRepr): Option[TypeRepr] =
      t.dealias match
        case AppliedType(tycon, s :: Nil) if tycon.typeSymbol.fullName == "io.bullet.borer.Encoder" =>
          Some(s)
        case _ => None
  ...
  def impl[T: Type](using q: Quotes): Expr[Registry[? <: Tuple, Encoder[T] *: EmptyTuple]] =
    ...
    def mkCtorData(childSym: Symbol): CtorData =
      ...
      if isModule then CtorData(childSym, displayName, true, Nil, Nil)
      else
        val ctor = childSym.primaryConstructor
        if ctor == Symbol.noSymbol then CtorData(childSym, displayName, true, Nil, Nil)
        else
    ...
    val prefixInputTypes: List[TypeRepr] = List(TypeRepr.of[CborOptions], TypeRepr.of[ConstructorEncoder])
    ...
    def buildMatch(value: Expr[T], encoders: Expr[Seq[Encoder[Any]]]): Expr[FromConstructor] =
      val cases: List[CaseDef] = ctorData.zipWithIndex.map: (c, idx) =>
        ...
        if c.isSingleton then
          ...
          val valueExprs: List[Expr[Dom.Element]] = c.fieldNames.zip(c.fieldTypes).map { (fn, ft) =>
            val encIdx = allFieldTypes.indexWhere(_ =:= ft)
            val encIdxExpr = Expr(encIdx)
            val selected: Term = Select.unique(bindRef, fn)
            ft.asType match
              case '[p] =>
                val fieldExpr: Expr[p] = selected.asExprOf[p]
                '{
                  toElement[p](
                    ${ encoders }.apply(${ encIdxExpr }).asInstanceOf[Encoder[p]],
                    $fieldExpr
                  )
                }
          }

          val valueListExpr: Expr[List[Dom.Element]] = Expr.ofList(valueExprs)
    ...
    val closure: Expr[Seq[Any] => Any] = '{ (args: Seq[Any]) =>
      ...
      Encoder[T] { (w: Writer, value: T) =>
        val fc: FromConstructor = ${ buildMatch('value, 'encoders) }
        val elem: Dom.Element = ce.encodeConstructor(opts, fc)
        w.write(elem)
      }
    }
