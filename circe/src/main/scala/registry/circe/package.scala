...
import io.circe.{ACursor, Decoder, Encoder, KeyDecoder, KeyEncoder}
...
package object circe:
  ...
  /** Summon a `KeyEncoder[A]` and register it. */
  inline def keyEncoderOf[A](using
      ce: KeyEncoder[A],
      tag: Tag[KeyEncoder[A]]
  ): TypedEntry[EmptyTuple, KeyEncoder[A]] =
    KeyEncoders.keyEncoderOf[A]

  /** Summon a `KeyDecoder[A]` and register it. */
  inline def keyDecoderOf[A](using
      cd: KeyDecoder[A],
      tag: Tag[KeyDecoder[A]]
  ): TypedEntry[EmptyTuple, KeyDecoder[A]] =
    KeyDecoders.keyDecoderOf[A]
  ...
  inline def encodeKey[A](f: A => String)(using tag: Tag[KeyEncoder[A]]): TypedEntry[EmptyTuple, KeyEncoder[A]] =
    KeyEncoders.encodeKey(f)

  /** Build a `KeyDecoder[A]` from a parsing function. */
  inline def decodeKey[A](f: String => Option[A])(using
      tag: Tag[KeyDecoder[A]]
  ): TypedEntry[EmptyTuple, KeyDecoder[A]] =
    KeyDecoders.decodeKey(f)
  ...
  ): TypedEntry[KeyEncoder[K] *: Encoder[V] *: EmptyTuple, Encoder[scala.collection.immutable.TreeMap[K, V]]] =
  ...
  inline def decodeTreeMapOf[K, V](using
      ...
      Tag[V],
      Ordering[K]
  ): TypedEntry[KeyDecoder[K] *: Decoder[V] *: EmptyTuple, Decoder[scala.collection.immutable.TreeMap[K, V]]] =
