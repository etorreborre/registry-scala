 ...
 *     variant's primary constructor or its singleton value) + a default `Chooser.uniform`.
 */
 ...
 *   - `const[T]` — pins one sampled value ACROSS ALL `makeGen` calls: every consumer in every
 *     `makeGen` call on that registry observes the SAME sampled value, regardless of seed. Use
 *     this for "fixture" data that is shared across the entire test run.
 *
 * {{{
 *   memoize[Version] +: gen(genVersion)   // cache the Gen[Version] instance across makeGen calles
 *   share[Version]   +: gen(genVersion)   // pin sample for all consumers within one makeGen call
 *   const[Version]   +: gen(genVersion)   // pin sample for all consumers across all makeGen calls
 * }}}
 */
